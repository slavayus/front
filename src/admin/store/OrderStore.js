...
const OrderStore = {
    ...
    setUpConnection() {
        ...
        const listenTypeQueue = "/queue/admin_" + uniqueUserId;
        ...
        function on_message(m) {
            ...
            switch (dataFromClient.status) {
                ...
                case 'empty': {
                    store.dispatch({type: 'CLEAR', data: 'Молодес, одмен. Список пуст.'});
                    break;
                }
                case 'error': {
    ...
    loadProducts() {
        axios.get(`${apiPrefix}:${serverPort}/admin/orders?queueId=${uniqueUserId}`, {withCredentials: true})
...
function products(state = {
    status: false,
    data: 'Молодес, одмен. Список пуст.'
}, action) {
    switch (action.type) {
        case 'LOAD':
            return {
